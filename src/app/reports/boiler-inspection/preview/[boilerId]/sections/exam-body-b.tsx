...
export function ExamBodyB({
      ...
      setDoc(
        docManometerRef,
        {
          ultrasoundTests: {
            ...ultrasoundTests,
            bodyExaminationB: {
              ...ultrasoundTests?.bodyExaminationB,
              photos: ultrasoundTests?.bodyExaminationB?.photos?.map((doc) =>
  ...
  return (
    <div className="w-full flex items-center justify-center flex-wrap">
      {ultrasoundTests?.bodyExaminationB?.photos &&
        ultrasoundTests?.bodyExaminationB?.photos.map((image, index) => {
          return (
            <ImageResizer
              key={image.id}
              image={image}
