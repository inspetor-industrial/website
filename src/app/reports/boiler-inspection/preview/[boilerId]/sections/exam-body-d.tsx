...
export function ExamBodyD({
      ...
      setDoc(
        docManometerRef,
        {
          ultrasoundTests: {
            ...ultrasoundTests,
            bodyExaminationD: {
              ...ultrasoundTests?.bodyExaminationD,
              photos: ultrasoundTests?.bodyExaminationD?.photos?.map((doc) =>
  ...
  return (
    <div className="w-full flex items-center justify-center flex-wrap">
      {ultrasoundTests?.bodyExaminationD?.photos &&
        ultrasoundTests?.bodyExaminationD?.photos.map((image, index) => {
          return (
            <ImageResizer
              key={image.id}
              image={image}
