...
export function FurnacePhotos({
    ...
    await Promise.all([
      setDoc(
        docManometerRef,
        {
          internalExaminationsPerformed: {
            ...internalExaminationsPerformed,
            furnace: internalExaminationsPerformed?.furnace?.map((doc) =>
  ...
  return (
    <div className="w-full flex items-center justify-center flex-wrap">
      {internalExaminationsPerformed?.furnace &&
        internalExaminationsPerformed?.furnace.map((image, index) => {
          return (
            <ImageResizer
              key={image.id}
              image={image}
