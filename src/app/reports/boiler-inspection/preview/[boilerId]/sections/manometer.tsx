...
type ManometerPhotosProps = {
  pressureGaugeCalibration: BoilerInspection['pressureGaugeCalibration']
  boilerId: string
  isDownload: boolean
}

export function ManometerPhotos({
  pressureGaugeCalibration,
  boilerId,
  isDownload,
}: ManometerPhotosProps) {
    ...
    await Promise.all([
      setDoc(
        docManometerRef,
        {
          pressureGaugeCalibration: {
            ...pressureGaugeCalibration,
            photos: pressureGaugeCalibration?.photos?.map((doc) =>
  ...
  return (
    <div className="w-full flex items-center justify-center flex-wrap">
      {pressureGaugeCalibration?.photos &&
        pressureGaugeCalibration?.photos.map((image, index) => {
          return (
            <ImageResizer
              key={image.id}
              image={image}
