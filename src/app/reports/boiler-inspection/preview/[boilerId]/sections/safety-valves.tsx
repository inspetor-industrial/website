...
type SafetyValvePhotosProps = {
  safetyValveGauge: BoilerInspection['safetyValveGauge']
  boilerId: string
  isDownload: boolean
}

export function SafetyValvePhotos({
  safetyValveGauge,
  boilerId,
  isDownload,
}: SafetyValvePhotosProps) {
    ...
    await Promise.all([
      setDoc(
        docManometerRef,
        {
          safetyValveGauge: {
            ...safetyValveGauge,
            photos: safetyValveGauge?.photos?.map((doc) =>
  ...
  return (
    <div className="w-full flex items-center justify-center flex-wrap">
      {safetyValveGauge?.photos &&
        safetyValveGauge?.photos.map((image, index) => {
          return (
            <ImageResizer
              key={image.id}
              image={image}
