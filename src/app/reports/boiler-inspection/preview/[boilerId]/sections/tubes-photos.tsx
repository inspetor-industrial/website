...
export function TubesPhotos({
    ...
    await Promise.all([
      setDoc(
        docManometerRef,
        {
          internalExaminationsPerformed: {
            ...internalExaminationsPerformed,
            tubes: internalExaminationsPerformed?.tubes?.map((doc) =>
  ...
  return (
    <div className="w-full flex items-center justify-center flex-wrap">
      {internalExaminationsPerformed?.tubes &&
        internalExaminationsPerformed?.tubes.map((image, index) => {
          return (
            <ImageResizer
              key={image.id}
              image={image}
