...
type ExamBodyCProps = {
  ultrasoundTests: BoilerInspection['ultrasoundTests']
  boilerId: string
  isDownload: boolean
}

export function ExamBodyC({
  ultrasoundTests,
  boilerId,
  isDownload,
}: ExamBodyCProps) {
      ...
      setDoc(
        docManometerRef,
        {
          ultrasoundTests: {
            ...ultrasoundTests,
            bodyExaminationC: {
              ...ultrasoundTests?.bodyExaminationC,
              photos: ultrasoundTests?.bodyExaminationC?.photos?.map((doc) =>
  ...
  return (
    <div className="w-full flex items-center justify-center flex-wrap">
      {ultrasoundTests?.bodyExaminationC?.photos &&
        ultrasoundTests?.bodyExaminationC?.photos.map((image, index) => {
          return (
            <ImageResizer
              key={image.id}
              image={image}
