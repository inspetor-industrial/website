...
type OperatorCertificatesProps = {
  operator: BoilerInspection['operator']
  boilerId: string
  isDownload: boolean
}

export function OperatorCertificates({
  operator,
  boilerId,
  isDownload,
}: OperatorCertificatesProps) {
    ...
    await Promise.all([
      setDoc(
        docManometerRef,
        {
          operator: {
            ...operator,
            certificate: operator?.certificate?.map((doc) =>
  ...
  return (
    <div className="w-full flex items-center justify-center flex-wrap">
      {operator.certificate &&
        operator.certificate.map((image, index) => {
          return (
            <ImageResizer
              key={image.id}
              image={image}
