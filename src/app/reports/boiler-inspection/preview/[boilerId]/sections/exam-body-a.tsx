...
export function ExamBodyA({
      ...
      setDoc(
        docManometerRef,
        {
          ultrasoundTests: {
            ...ultrasoundTests,
            bodyExaminationA: {
              ...ultrasoundTests?.bodyExaminationA,
              photos: ultrasoundTests?.bodyExaminationA?.photos?.map((doc) =>
  ...
  return (
    <div className="w-full flex items-center justify-center flex-wrap">
      {ultrasoundTests?.bodyExaminationA?.photos &&
        ultrasoundTests?.bodyExaminationA?.photos.map((image, index) => {
          return (
            <ImageResizer
              key={image.id}
              image={image}
